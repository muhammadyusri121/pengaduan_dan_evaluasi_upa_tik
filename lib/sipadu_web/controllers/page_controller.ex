defmodule SipaduWeb.PageController do
  ...
  defp get_default_faq() do
    [
      ...
      %{
        ...
        a:
          "Silakan buat laporan dengan memilih kategori 'Perangkat Keras'. Pastikan Anda menyebutkan lokasi alat (Nama Gedung & No Ruangan) agar teknisi mudah mencarinya."
      },
      %{
        id: "d3",
        ...
        a:
          "Buat tiket laporan pada kategori ini dengan menyebutkan nama software yang dibutuhkan. Tim kami akan merespon dengan tautan instalasi atau panduan."
      },
      %{
        id: "d4",
        kategori: "Akun & Email",
        ...
        a:
          "Jika fitur 'Lupa Password' gagal, ajukan laporan di kategori 'Akun & Email' dengan menyertakan foto KTM untuk proses verifikasi reset password."
      },
      %{
        id: "d5",
        ...
        a:
          "Bisa jadi server sedang dalam masa maintenance. Jika error terus berlanjut, mohon lampirkan screenshot pesan error dan URL halaman yang bermasalah."
      },
      %{
        id: "d6",
        kategori: "Lainnya",
        ...
        a:
          "Silakan pilih kategori 'Lainnya' dan jelaskan selengkap mungkin kronologi masalah Anda. Tim teknis akan menganalisis dan mengarahkannya ke divisi yang tepat."
      }
    ]
  end
  ...
  def home(conn, _params) do
    ...
    kategori =
      Enum.map(kategori_db, fn k ->
        icon =
          cond do
            String.match?(String.downcase(k.nama), ~r/jaringan|wifi|internet/) ->
              "hero-wifi"

            String.match?(String.downcase(k.nama), ~r/sistem|akademik|portal/) ->
              "hero-academic-cap"

            String.match?(String.downcase(k.nama), ~r/email|surat/) ->
              "hero-envelope"

            String.match?(String.downcase(k.nama), ~r/perangkat|hardware|komputer|printer/) ->
              "hero-computer-desktop"

            String.match?(String.downcase(k.nama), ~r/software|aplikasi|lisensi/) ->
              "hero-cube"

            String.match?(String.downcase(k.nama), ~r/web/) ->
              "hero-globe-alt"
  ...
  def cari(conn, %{"q" => query}) do
    ...
    mapped_default =
      get_default_faq()
      |> Enum.filter(fn d ->
        String.contains?(String.downcase(d.q), search_term) or
          String.contains?(String.downcase(d.a), search_term)
      end)
      |> Enum.map(fn d -> %{judul_laporan: d.q, tanggapan_admin: d.a} end)
