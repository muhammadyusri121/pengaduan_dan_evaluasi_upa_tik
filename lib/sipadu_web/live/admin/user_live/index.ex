  ...
  def render(assigns) do
    ...
    <div class="space-y-6">
      <div>
        ...
        <p class="text-sm text-slate-500 mt-1">Kelola hak akses pengguna platform SIPADU.</p>
      </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse">
            ...
            <tbody id="user-table" phx-update="stream" class="divide-y divide-slate-150">
              <tr :for={{id, user} <- @streams.user_list} id={id} class="hover:bg-slate-50/50 transition">
                <td class="px-6 py-4 whitespace-nowrap">
                  <img src={user.image} class="w-10 h-10 rounded-full border border-slate-200" alt="Avatar" />
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                  <%= if user.id != @current_user.id do %>
                    <button
                      ...
                      class={[
                        "inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold transition border",
